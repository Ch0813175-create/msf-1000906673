...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun triggerAutoFixAndRecompile(project: AppProject) {
        ...
        viewModelScope.launch {
            try {
                _uploadingStatus.value = "🤖 Fetching error logs and diagnosing repository..."
                _uploadProgress.value = 15

                // 1. Fetch live jobs & step logs if available
                var logSummary = project.errorMessage ?: ""
                if (project.workflowRunId > 0L) {
                    val jobsResult = gitHubRepo.fetchWorkflowRunJobs(token, project.repoOwner, project.repoName, project.workflowRunId)
                    if (jobsResult.isSuccess) {
                        val jobsList = jobsResult.getOrNull() ?: emptyList()
                        _liveJobs.value = jobsList
                        val failedSteps = jobsList.flatMap { it.steps }.filter { it.conclusion == "failure" || it.status == "in_progress" }
                        if (failedSteps.isNotEmpty()) {
                            logSummary += "\n" + failedSteps.joinToString("\n") { "Step: ${it.name} | Status: ${it.status} | Conclusion: ${it.conclusion}" }
                        }
                    }
                }

                _uploadingStatus.value = "🤖 Executing self-healing code algorithms..."
                _uploadProgress.value = 35

                // 2. Build template file fallback map and apply auto-fix algorithms
                val baseFiles = ZipUtils.getSampleProjectFiles("compose_counter").map { entry ->
                    ZipUtils.FileEntry(entry.key, entry.value)
                }
                val fixResult = BuildDiagnosticsEngine.autoFixProjectFromLogs(baseFiles, logSummary, project.appName)
                val filesToUpload = fixResult.patchedFiles.associate { file -> file.relativePath to file.content }

                _uploadingStatus.value = "🤖 Uploading patched code to GitHub..."
                _uploadProgress.value = 50

                val uploadResult = gitHubRepo.uploadCodeFiles(token, project.repoOwner, project.repoName, filesToUpload) { prog, msg ->
                ...
                if (uploadResult.isSuccess) {
                    projectDao.updateProjectStatus(
                        ...
                        downloadUrl = null,
                        workflowRunId = 0,
                        errorMessage = "Self-healing patches applied! Re-compiling assembled release APK..."
                    )
                    // Restart polling
                    val updatedProject = project.copy(
                        ...
                        downloadUrl = null,
                        workflowRunId = 0,
                        errorMessage = "Self-healing patches applied! Re-compiling..."
                    )
                    pollProjectStatusManually(updatedProject)
                } else {
                    _uploadingStatus.value = "Auto-fix commit failed: ${uploadResult.exceptionOrNull()?.localizedMessage}"
                }
            } catch (e: Exception) {
